 ...
 * @package Bss\ProductStockAlert\Model
...
class StockEmailProcessor
{
    ...
    protected function _processStock(
        ...
        $affectedProductIds = $affectedData['affectedProductIds'];
    ...
    private function retrieveAffectedData()
    {
        $dataStock = [];
        $affectedProductIds = [];
        foreach (array_chunk($this->stockNotiCollection->getData(), self::BATCH_SIZE) as $chunkCollection) {
            // Split collection to chunk
            foreach ($chunkCollection as $stockModel) {
    ...
    private function saveModel(
        ...
        if ($checkModelSave == 1) {
            $customerNames[$customerEmail] = $this->getCustomerName($customerId, $customerEmail);
            ...
            if ($this->isConfigurableAndHasChild($product->getTypeId(), $productId)) {
                ...
                foreach ($this->productRelation[$productId] as $childProductId) {
                    ...
                    $childIsInStock = $childProductStock->getIsInStock();
                    $qtyToSend = $this->helper->getQtySendMail($storeId);
                    $childQty = $childProductStock->getQty();
    ...
    private function retrievePidsListNotExist($affectedProductIds, $affectedData)
    {
        ...
        if (!empty($affectedNoticeIds)) {
            $this->errors[] = __("We found list of product ids that does not exist. Invalid product ids: %1", implode(',', $affectedProductIds));
        }
    ...
    private function createBulkStockItems($stockItems)
    {
        if (!$this->stockItems) {
            $items = [];
            foreach (array_chunk($stockItems, self::BATCH_SIZE) as $chunkStockItems) {
                /** @var StockItemInterface $stockItem */
                foreach ($chunkStockItems as $stockItem) {
                    $items[$stockItem->getProductId()] = $stockItem;
                }
            }
    ...
    private function setMessageErrors($message, $alertProductId)
    {
        if (!isset($this->errors[$alertProductId])) {
            if ($message == __("The product that was requested doesn't exist. Verify the product and try again.")) {
                $messageError = __("- The product ID ") . $alertProductId . __(" that was requested does not exist. Verify the product and try again.");
