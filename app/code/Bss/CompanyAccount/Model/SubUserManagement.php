...
class SubUserManagement implements SubUserManagementInterface
{
        ...
        try {
            ...
            if ($subUsers) {
                foreach ($subUsers as $subUser) {
                    $customer = $this->customerRepository->getById($subUser->getCompanyCustomerId());
    ...
    public function resetPassword($subId, $token, $password, $websiteId)
    {
        try {
            ...
            $subUser->setSubUserPassword($this->createPasswordHash($password));
            $subUser->setSubUserToken(null);
    ...
    public function authenticate($subUser, $password)
    {
        ...
        return $this->encryptor->validateHash($password, $subUser->getSubUserPassword());
    }
