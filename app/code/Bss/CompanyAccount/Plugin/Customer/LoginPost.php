...
class LoginPost
{
    ...
    public function __construct(
        ...
        $this->objectManager = $this->helper->getDataHelper()->getObjectManager();
            ...
            if ($this->helper->isEnable($websiteId) && $subUser) {
                if ($subject->getRequest()->isPost()) {
                    if (!empty($login['username']) && !empty($login['password'])) {
                        ...
                        $validatedResult = $this->validateB2bRegistration($customer);
                        if ($validatedResult) {
                            $this->session->setUsername($login['username']);
                            $this->messageManager->addErrorMessage($validatedResult);
                            return $this->redirectFactory->create()
                                ->setPath('*/*/');
                        }

                        $isAuthenticated = $this->subUserManagement->authenticate($subUser, $login['password']);
                        if ($isAuthenticated && !$subUser->getSubUserStatus()) {
                            $resultRedirect = $this->redirectFactory->create();
                            $this->messageManager->addErrorMessage(__('Your account is inactive. Please contact your company account for more details.'));
                            $this->session->setUsername($login['username']);
                            return $resultRedirect->setPath('*/*/');
                        }
                        if ($isAuthenticated && $subUser->getSubUserStatus()) {
