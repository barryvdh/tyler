    ...
    public function getConfig()
    {
        ...
        if ($attributeHelper->getConfig('bss_customer_attribute/general/enable')) {
            ...
            foreach ($addressCollection as $attribute) {
                if ($attribute->getIsRequired() == 1 &&
                    $attributeHelper->isAddressAddToCheckout($attribute->getAttributeCode())
                ) {
                    switch ($attribute->getFrontendInput()) {
                        case "multiselect":
                            $config[] = "select[name='custom_attributes["
                                . $attribute->getAttributeCode() . "][]']";
                            break;
                        case "boolean":
                        case "select":
                            ...
                            break;
                        case "textarea":
                            $config[] = "textarea[name='custom_attributes["
                                . $attribute->getAttributeCode() . "]']";
                            break;
                        default:
                            $config[] = "input[name='custom_attributes[" . $attribute->getAttributeCode() . "]']";
                    }
                }
            }
