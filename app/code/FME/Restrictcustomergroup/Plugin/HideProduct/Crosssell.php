<?php
/**
 * Class for Restrictcustomergroup HideProduct Crosssell
...
class Crosssell
{
    ...
    public function afterGetItems(
        \Magento\Catalog\Block\Product\ProductList\Crosssell $subject,
         $result
    )
    {
      ...
      $crosssellprodcollection = $result;
      if (!$this->_restrictcustomergroupHelper->isEnabledInFrontend())
      {
        return $crosssellprodcollection;
      }
      ...
      foreach ($crosssellprodcollection as $key => $product)
      {
        $ruleProducts = [];
        ...
        if (!empty($collection->getData()))
        {
          foreach ($collection->getData() as $item)
          {
            ...
            {
              $crosssellprodcollection->removeItemByKey($key);
            }
            else
            {
