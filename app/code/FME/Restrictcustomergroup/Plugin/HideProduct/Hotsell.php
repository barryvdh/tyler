<?php
/**
 * Class for Restrictcustomergroup HideProduct Hotsell
...
class Hotsell
{
    ...
    )
    {
      ...
      $hotesellprodcollection = $result;
      if (!$this->_restrictcustomergroupHelper->isEnabledInFrontend())
      {
          return $hotesellprodcollection;
      }
      ...
      foreach ($hotesellprodcollection as $key => $product)
      {
        $ruleProducts = [];
        ...
        if (!empty($collection->getData()))
        {
          foreach ($collection->getData() as $item)
          {
            ...
            {
              $hotesellprodcollection->removeItemByKey($key);
            }
            else
            {
