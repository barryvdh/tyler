<?php
/**
 * Class for Restrictcustomergroup HideProduct Toolbar
...
class Toolbar
{
    ...
    )
    {
      ...
      $toolbarprodcollection = $result;
      if (!$this->_restrictcustomergroupHelper->isEnabledInFrontend())
      {
          return $toolbarprodcollection;
      }
      ...
      foreach ($toolbarprodcollection as $key => $product)
      {
        $ruleProducts = [];
        ...
        if (!empty($collection->getData()))
        {
          foreach ($collection->getData() as $item)
          {
            ...
            {
              $toolbarprodcollection->removeItemByKey($key);
            }
            else
            {
