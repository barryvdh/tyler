<?php
/**
 * Class for Restrictcustomergroup HideProduct Compare Main
 * @Copyright © FME fmeextensions.com. All rights reserved.
 * @author Arsalan Ali Sadiq <support@fmeextensions.com>
 * @package FME Restrictcustomergroup
 * @license See COPYING.txt for license details.
 */
namespace FME\Restrictcustomergroup\Plugin\HideProduct\Compare;
...
class Main
{
    ...
    )
    {
      ...
      $compareprodcollection = $result;
      if (!$this->_restrictcustomergroupHelper->isEnabledInFrontend())
      {
          return $compareprodcollection;
      }
      ...
      foreach ($compareprodcollection as $key => $compareitem)
      {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $product = $objectManager->create('Magento\Catalog\Model\Product')->load($compareitem->getProductId());
        $ruleProducts = [];
        ...
        if (!empty($collection->getData()))
        {
          foreach ($collection->getData() as $item)
          {
            ...
            {
              $compareprodcollection->removeItemByKey($key);
            }
            else
            {
