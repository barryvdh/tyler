<?php
/**
 * Class for Restrictcustomergroup HideProduct Wishlist Main
...
class Main
{
    ...
    public function afterGetWishlistItems(
        \Magento\Wishlist\Block\Customer\Wishlist $subject,
         $result
    )
    {
      ...
      $wishlistprodcollection = $result;
      if (!$this->_restrictcustomergroupHelper->isEnabledInFrontend())
      {
          return $wishlistprodcollection;
      }
      ...
      foreach ($wishlistprodcollection as $key => $wishlistitem)
      {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $product = $objectManager->create('Magento\Catalog\Model\Product')->load($wishlistitem->getProductId());
        $ruleProducts = [];
        ...
        if (!empty($collection->getData()))
        {
          foreach ($collection->getData() as $item)
          {
            ...
            {
              $wishlistprodcollection->removeItemByKey($key);
            }
            else
            {
